...
def model(train_data, test_data, epochs, limits, version=0.0):
    """
    ...
    """
    # data loader
    train_dataloader = ColorizeDataLoader(train_data, limit=limits[0])
    train_dataloader = DataLoader(
        train_dataloader, batch_size=config.BATCH_SIZE,
        shuffle=True, num_workers=2, drop_last=True)

    test_dataloader = ColorizeDataLoader(test_data, limit=limits[1])
    ...
    metrics = {'epoch': [], 'g_loss': [], 'd_loss': [], 'MSE': []}

    for epoch in range(epochs):
        print(f'EPOCH {epoch} / {epochs}')
        print('-' * 30)

        g_loss_avg, d_loss_avg, MSE_loss_avg = [], [], []
        for idx, (trainL, trainAB, _) in enumerate(tqdm(train_dataloader)):
            ...
            MSE_loss_avg.append(MSE_loss.detach().cpu().float().item())
            W_loss = wasserstein_loss(dis_C, True) * 0.1
            g_loss = KLD_loss + MSE_loss + W_loss
            g_loss_avg.append(g_loss.detach().cpu().float().item())
            ...
            d_loss_avg.append(d_loss.detach().cpu().float().item())
        ...
        metrics['g_loss'].append(np.mean(g_loss_avg))
        metrics['d_loss'].append(np.mean(d_loss_avg))
        metrics['MSE'].append(np.mean(MSE_loss_avg))
    ...
    with open("evals/metrics.json", 'w') as file:
        json.dump(metrics, file)
        print("Saving metrics successfully!")



def train(limits: list):
    ...
    model(train_path, test_path, epochs, limits=limits)
...
def sample_images(test_data, colorizationModel):
    ...
    for idx, (gray, ori_ab, _) in enumerate(tqdm(test_data)):
        ...
        for i in range(config.BATCH_SIZE):
            ...
            cv2.imwrite(config.OUTPUT_PATH + str(idx) + '.jpg', original_result_red)
    print('Sampling images done')


def test(limits: list):
    """
    Test the model
    """
    path = config.MODEL_C
    test_dataloader = ColorizeDataLoader(config.TEST_PATH, limit=limits[1])
    test_dataloader = DataLoader(
        test_dataloader, batch_size=config.BATCH_SIZE,
        shuffle=True, num_workers=2, drop_last=True)
